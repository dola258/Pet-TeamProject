	...
	<div class="card">
		<div class="card-header">
				...
				<div class="btn-group">
					<div class="btn-group">
						...
						<ul class="dropdown-menu">
							<li><a class="dropdown-item" href="/2/tip/${tipEntity.id}/updateForm">수정하기</a></li>
							<li><a class="dropdown-item" onclick="deleteById(${tipEntity.id})">삭제하기</a></li>
						</ul>
					</div>
				</div>
			</c:if>
			...
			<div class="d-flex font-size">
				<div class="p-2"><small>글 작성자 : ${tipEntity.user.username}</small></div>
				<div class="p-2"><small>${parseCreatedAt}</small></div>
				<div class="p-2 "><small>조회 ${tipEntity.counter}</small></div>
				<div class="flex-fill"></div>
			</div>
		</div>
		<div class="card-body" style="overflow: auto;">${tipEntity.content}</div>
		<div class="card">
			<div class="card-header" style="text-align: center;">댓글 리스트</div>
			<p></p>
			<c:forEach var="comment" items="${tipEntity.comments }" >
			...
			<form action="/api/2/tip/${tipEntity.id}/comment" method="post">
...
<script>
	async function deleteById(id) {
		...
		// 1. 비동기 함수 호출
		let response = await fetch("http://localhost:8080/api/tip/"+id, {
			method: "delete"
		});
